...
FastAPI backend with Streamlit client.  
...
- Reason: Selenium is the tool I'm most familiar with and Beautiful Soup is an excellent HTML parser, on top of that, the pages dividendhistory.org are loaded in using Javascript and so using Beautiful Soup alone is not viable
...
- Reason: dividends are announced at most quarterly, so a 30-day refresh guarantees that at least one refresh per announcement cycle
